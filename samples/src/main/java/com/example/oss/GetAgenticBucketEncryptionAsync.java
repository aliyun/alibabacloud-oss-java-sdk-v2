...
public class GetAgenticBucketEncryptionAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncAgenticBucketClient client = clientBuilder.build()) {
            GetAgenticBucketEncryptionResult result = client.getAgenticBucketEncryptionAsync(GetAgenticBucketEncryptionRequest.newBuilder()
                    .bucket(bucket)
                    .build()).get();

            ServerSideEncryptionRule rule = result.serverSideEncryptionRule();
            if (rule != null && rule.applyServerSideEncryptionByDefault() != null) {
                System.out.printf("sse algorithm:%s, kms master key id:%s\n",
