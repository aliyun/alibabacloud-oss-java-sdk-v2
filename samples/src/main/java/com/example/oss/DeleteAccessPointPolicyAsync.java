...
public class DeleteAccessPointPolicyAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            DeleteAccessPointPolicyResult result = client.deleteAccessPointPolicyAsync(DeleteAccessPointPolicyRequest.newBuilder()
                            .bucket(bucket)
                            .accessPointName(name)
                            .build()).get();

            System.out.printf("Delete access point policy async success, request id: %s\n",
                    result.requestId());
