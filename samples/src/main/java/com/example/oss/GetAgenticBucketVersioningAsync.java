...
public class GetAgenticBucketVersioningAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncAgenticBucketClient client = clientBuilder.build()) {
            GetAgenticBucketVersioningResult result = client.getAgenticBucketVersioningAsync(GetAgenticBucketVersioningRequest.newBuilder()
                    .bucket(bucket)
                    .build()).get();

            VersioningConfiguration configuration = result.versioningConfiguration();
            if (configuration != null) {
                System.out.printf("versioning status:%s\n", configuration.status());
            }
