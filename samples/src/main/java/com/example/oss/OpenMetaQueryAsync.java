...
public class OpenMetaQueryAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            // Prepare the meta query filters
            MetaQueryFilters filters = MetaQueryFilters.newBuilder()
                    .filter(Arrays.asList("Size > 1024, FileModifiedTime > 2025-06-03T09:20:47.999Z"))
                    .build();

            // Execute open meta query asynchronously
            OpenMetaQueryResult result = client.openMetaQueryAsync(
