...
public class PutAgenticBucketStatusAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncAgenticBucketClient client = clientBuilder.build()) {
            PutAgenticBucketStatusResult result = client.putAgenticBucketStatusAsync(PutAgenticBucketStatusRequest.newBuilder()
                    .bucket(bucket)
    ...
    public Options getOptions() {
        ...
        opts.addOption(Option.builder().longOpt("status").desc("The status of the agentic bucket, e.g. Enabled, Disabled.").hasArg().required().get());
        return opts;
    }
