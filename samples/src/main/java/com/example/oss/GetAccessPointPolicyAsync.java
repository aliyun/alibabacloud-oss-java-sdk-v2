...
public class GetAccessPointPolicyAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            GetAccessPointPolicyResult result = client.getAccessPointPolicyAsync(GetAccessPointPolicyRequest.newBuilder()
                            .bucket(bucket)
                            .accessPointName(name)
                            .build()).get();

            System.out.printf("Get access point policy async success, request id: %s\n", result.requestId());
