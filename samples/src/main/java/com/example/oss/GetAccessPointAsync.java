...
public class GetAccessPointAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            ...
            GetAccessPointResult result = client.getAccessPointAsync(GetAccessPointRequest.newBuilder()
                            .bucket(bucket)
                            .accessPointName(name)
                            .build()).get();

            System.out.printf("Get access point async success, name: %s, bucket: %s, network origin: %s, request id: %s\n",
