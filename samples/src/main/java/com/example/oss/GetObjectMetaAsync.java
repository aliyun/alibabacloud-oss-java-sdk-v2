...
public class GetObjectMetaAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            ...
            GetObjectMetaResult result = client.getObjectMetaAsync(GetObjectMetaRequest.newBuilder()
                            .bucket(bucket)
                            .key(key)
                    .build()).get();

            System.out.printf("status code:%d, request id:%s, eTag:%s, last modified:%s, content length:%d\n",
                    result.statusCode(), result.requestId(), result.eTag(), result.lastModified(), result.contentLength());
