...
public class GetBucketPolicyAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            ...
            GetBucketPolicyResult result = client.getBucketPolicyAsync(GetBucketPolicyRequest.newBuilder()
                            .bucket(bucket)
                            .build()).get();
            ...
            if (result.body() != null) {
                System.out.println("Bucket policy:");
                System.out.println(result.body().toString());
            } else {
                System.out.println("No bucket policy found.");
            }
