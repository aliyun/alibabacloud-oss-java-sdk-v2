...
public class GetAgenticBucketPublicAccessBlockAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncAgenticBucketClient client = clientBuilder.build()) {
            GetAgenticBucketPublicAccessBlockResult result = client.getAgenticBucketPublicAccessBlockAsync(GetAgenticBucketPublicAccessBlockRequest.newBuilder()
                    .bucket(bucket)
                    .build()).get();

            PublicAccessBlockConfiguration configuration = result.publicAccessBlockConfiguration();
            if (configuration != null) {
                System.out.printf("block public access:%s\n", configuration.blockPublicAccess());
            }
