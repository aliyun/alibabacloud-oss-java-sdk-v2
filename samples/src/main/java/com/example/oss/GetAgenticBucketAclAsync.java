...
public class GetAgenticBucketAclAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncAgenticBucketClient client = clientBuilder.build()) {
            GetAgenticBucketAclResult result = client.getAgenticBucketAclAsync(GetAgenticBucketAclRequest.newBuilder()
                    .bucket(bucket)
                    .build()).get();

            AccessControlPolicy policy = result.accessControlPolicy();
            if (policy != null && policy.accessControlList() != null) {
                System.out.printf("acl:%s\n", policy.accessControlList().grant());
            }
