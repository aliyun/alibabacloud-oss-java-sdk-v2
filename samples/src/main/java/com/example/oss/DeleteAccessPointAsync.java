...
public class DeleteAccessPointAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            DeleteAccessPointResult result = client.deleteAccessPointAsync(DeleteAccessPointRequest.newBuilder()
                            .bucket(bucket)
                            .accessPointName(name)
                            .build()).get();

            System.out.printf("Delete access point async success, request id: %s\n",
                    result.requestId());
