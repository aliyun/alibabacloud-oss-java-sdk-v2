...
public class ExtendBucketWormAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            ...
            ExtendBucketWormResult result = client.extendBucketWormAsync(request).get();
    ...
    public Options getOptions() {
        ...
        opts.addOption(Option.builder().longOpt("wormId").desc("The ID of the retention policy.").hasArg().required().get());
    ...
    public void runCmd(CommandLine cmd) throws ParseException {
        ...
        String wormId = cmd.getParsedOptionValue("wormId");
        Integer retentionPeriodInDays = Integer.parseInt(cmd.getParsedOptionValue("retentionPeriodInDays"));
        execute(endpoint, region, bucket, wormId, retentionPeriodInDays);
    }
}