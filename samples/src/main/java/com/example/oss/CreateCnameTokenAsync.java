...
public class CreateCnameTokenAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            ...
            CreateCnameTokenResult result = client.createCnameTokenAsync(request).get();

            System.out.printf("Status code:%d, request id:%s\n",
                    result.statusCode(), result.requestId());
            System.out.printf("Bucket:%s\n", result.cnameToken().bucket());
            System.out.printf("Cname:%s\n", result.cnameToken().cname());
            System.out.printf("Token:%s\n", result.cnameToken().token());
