...
public class GetPresignedUrl implements Example {
    private static void execute(
        ...
        try (ClientImpl client = new ClientImpl(config.build())) {

            GetObjectRequest request = GetObjectRequest.newBuilder().bucket(bucket).key(key).build();

            PresignResult result = Presigner.getObject(client, request, PresignOptions.defaults());
