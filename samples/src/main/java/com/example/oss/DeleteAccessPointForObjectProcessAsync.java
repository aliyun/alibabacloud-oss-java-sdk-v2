...
public class DeleteAccessPointForObjectProcessAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            ...
            CompletableFuture<DeleteAccessPointForObjectProcessResult> future = client.deleteAccessPointForObjectProcessAsync(request);

            DeleteAccessPointForObjectProcessResult result = future.get();

            System.out.printf("Delete access point for object process success, request id: %s\n",
                    result.requestId());
