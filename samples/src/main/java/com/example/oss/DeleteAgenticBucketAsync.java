...
public class DeleteAgenticBucketAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncAgenticBucketClient client = clientBuilder.build()) {
            // An agentic bucket can be deleted only after it has been disabled and all of
            // its bucket spaces have been removed.
            DeleteAgenticBucketResult result = client.deleteAgenticBucketAsync(DeleteAgenticBucketRequest.newBuilder()
                    .bucket(bucket)
                    .build()).get();
