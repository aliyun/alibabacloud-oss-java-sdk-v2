...
public class GetAccessPointPolicyForObjectProcessAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSAsyncClient client = getDefaultAsyncClient(endpoint, region, provider)) {
            ...
            CompletableFuture<GetAccessPointPolicyForObjectProcessResult> future = client.getAccessPointPolicyForObjectProcessAsync(request);

            GetAccessPointPolicyForObjectProcessResult result = future.get();

            System.out.printf("Get access point policy for object process async success, request id: %s\n",
                    result.requestId());
            
            if (result.body() != null) {
                System.out.printf("Policy document: %s\n", result.body().toString());
            }
