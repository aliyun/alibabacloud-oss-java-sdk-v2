...
public class GetMetaQueryStatusAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSDataProcessAsyncClient client = clientBuilder.build()) {
            ...
            GetMetaQueryStatusResult result = client.getMetaQueryStatusAsync(
            ...
            if (result.metaQueryStatus() != null) {
                MetaQueryStatus status = result.metaQueryStatus();
                System.out.printf("State:%s%n", status.state());
                System.out.printf("Phase:%s%n", status.phase());
                System.out.printf("Create time:%s%n", status.createTime());
                System.out.printf("Update time:%s%n", status.updateTime());
            }
