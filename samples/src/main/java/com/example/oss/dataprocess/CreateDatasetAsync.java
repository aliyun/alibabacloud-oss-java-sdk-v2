...
public class CreateDatasetAsync implements Example {
    ...
    private static void execute(
        ...
        try (OSSDataProcessAsyncClient client = clientBuilder.build()) {
            ...
            CreateDatasetResult result = client.createDatasetAsync(requestBuilder.build()).get();

            System.out.printf("Status code:%d, request id:%s%n",
                    result.statusCode(), result.requestId());

            if (result.dataset() != null) {
                System.out.printf("Dataset name:%s%n", result.dataset().datasetName());
            }
    ...
    public Options getOptions() {
        ...
        opts.addOption(Option.builder().longOpt("description").desc("The description of the dataset.").hasArg().get());
        return opts;
    }
