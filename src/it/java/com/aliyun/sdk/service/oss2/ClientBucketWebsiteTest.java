...
public class ClientBucketWebsiteTest extends TestBase {
    ...
    public void testBucketWebsite() throws Exception {
        ...
        try {
            // 1. create bucket
            PutBucketResult bucketResult = client.putBucket(PutBucketRequest.newBuilder()
            ...
            PutBucketWebsiteResult putResult = client.putBucketWebsite(PutBucketWebsiteRequest.newBuilder()
                    .bucket(testBucketName)
            ...
            GetBucketWebsiteResult getResult = client.getBucketWebsite(GetBucketWebsiteRequest.newBuilder()
                    .bucket(testBucketName)
                    .build());
            ...
            DeleteBucketWebsiteResult deleteResult = client.deleteBucketWebsite(DeleteBucketWebsiteRequest.newBuilder()
                    .bucket(testBucketName)
                    .build());
    ...
    public void testBucketWebsiteNewRedirect() throws Exception {
        ...
        try {
            ...
            ReturnHeader returnHeader1 = ReturnHeader.newBuilder()
                    .key("header-key1")
                    .value("header-value1")
                    .build();

            ReturnHeader returnHeader2 = ReturnHeader.newBuilder()
                    .key("header-key2")
                    .value("header-value2")
                    .build();
            ...
            Assert.assertEquals("index.html", resultConfig.indexDocument().suffix());
            Assert.assertEquals(true, resultConfig.indexDocument().supportSubDir());
            Assert.assertEquals(Long.valueOf(0L), resultConfig.indexDocument().type());
            Assert.assertEquals("error.html", resultConfig.errorDocument().key());
            Assert.assertEquals(404, resultConfig.errorDocument().httpStatus().intValue());
