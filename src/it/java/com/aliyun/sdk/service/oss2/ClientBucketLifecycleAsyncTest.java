...
public class ClientBucketLifecycleAsyncTest extends TestBase {
    ...
    public void testBucketLifecycleAdvancedOperations() throws Exception {
        ...
        LifecycleRule abortMultipartRule = LifecycleRule.newBuilder()
                ...
                .prefix("multipart/")
                .status("Enabled")
                .abortMultipartUpload(LifecycleRuleAbortMultipartUpload.newBuilder().days(5).build())
                .build();
                ...
                .filter(LifecycleRuleFilter.newBuilder()
                        ...
                        .nots(Arrays.asList(
                                LifecycleRuleNot.newBuilder()
                                        ...
                                        .build()))
                        .build())
                .expiration(LifecycleRuleExpiration.newBuilder().days(10).build())
                .build();
        ...
        for (int i = 0; i < rules.size(); i++) {
            ...
            if (originalRule.transitions() != null) {
                ...
                for (int j = 0; j < originalRule.transitions().size(); j++) {
                    LifecycleRuleTransition originalTransition = originalRule.transitions().get(j);
                    LifecycleRuleTransition resultTransition = resultRule.transitions().get(j);
            ...
            if (originalRule.filter() != null) {
                ...
                if (originalRule.filter().nots() != null) {
                    Assert.assertNotNull("Rule filter nots should not be null", resultRule.filter().nots());
                    Assert.assertEquals("Rule filter nots count should match",
                            originalRule.filter().nots().size(), resultRule.filter().nots().size());

                    for (int j = 0; j < originalRule.filter().nots().size(); j++) {
                        LifecycleRuleNot originalNot = originalRule.filter().nots().get(j);
                        LifecycleRuleNot resultNot = resultRule.filter().nots().get(j);

                        if (originalNot.prefix() != null) {
                            Assert.assertEquals("Not prefix should match",
                                    originalNot.prefix(), resultNot.prefix());
                        }

                        if (originalNot.tag() != null) {
                            Assert.assertNotNull("filter not tag should not be null", resultNot.tag());
                            Assert.assertEquals("filter not tag key should match",
                                    originalNot.tag().key(), resultNot.tag().key());
                            Assert.assertEquals("filter not tag value should match",
