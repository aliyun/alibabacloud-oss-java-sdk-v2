...
/**
 * Implements the OSS V4 signature protocol for request signing (Tables).
 */
public class TablesSignerV4 implements Signer {
    /**
    ...
    private String buildCanonicalUri(SigningContext signingCtx) {
        ...
        if (!StringUtils.isNullOrEmpty(signingCtx.getBucket())) {
            ...
            uri.append(String.format("acs:osstables:%s::bucket/", signingCtx.getRegion())).append("/");
        }
        if (!StringUtils.isNullOrEmpty(signingCtx.getKey())) {
            uri.append(HttpUtils.urlDecode(signingCtx.getKey()));
        }
